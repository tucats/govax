...
// own ISIG-driven SIGINT generation is disabled — exactly the case here,
// since readline.NewEx puts the terminal into raw mode (ISIG off, among
// other things) for the entire life of the process, not just while
// actively reading a line. That means a real SIGINT is never generated by
// Ctrl-C once readline has started: attentionStdin (below) is what makes
// Ctrl-C interrupt a running VAX program anyway.

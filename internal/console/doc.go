...
// device-dependent commands (SHOW DEVICE, DEFINE/DEVICE, ...) are stubbed
// pending Phase 09 (I/O), RTL-entry-point commands (CALL, BOOT, ABOUT,
// FORTH, XTEST — anything reached via a DCL /entry=) are stubbed pending
// Phase 10 (RTL), and the inline mini-assembler (ASM/DISASM, and EXAMINE's
// address-expression syntax) is replaced by a small standalone expression
// evaluator (expr.go) rather than waiting on Phase 11's real assembler.
package console
